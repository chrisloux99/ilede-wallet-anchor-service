...
export const rfq = api<RfqRequest, RfqResponse>(
  ...
  async (req) => {
    try {
      // Validate input
      validate()
        .required("sell_asset", req.sell_asset)
        .required("buy_asset", req.buy_asset)
        .assetCode("sell_asset", req.sell_asset)
        .assetCode("buy_asset", req.buy_asset)
        .amount("sell_amount", req.sell_amount)
        .amount("buy_amount", req.buy_amount)
        .validate();

      // Validate that at least one amount is provided
      if (!req.sell_amount && !req.buy_amount) {
        throw new ValidationError("Amount required", {
          amount: "Either sell_amount or buy_amount must be specified"
        });
      }

      // Validate that sell and buy assets are different
      if (req.sell_asset === req.buy_asset) {
        throw new ValidationError("Invalid asset pair", {
          assets: "Sell and buy assets must be different"
        });
      }

      // Validate supported assets
      const supportedAssets = ["iLede", "USDC", "USD"];
      if (!supportedAssets.includes(req.sell_asset) || !supportedAssets.includes(req.buy_asset)) {
        throw new ValidationError("Unsupported asset", {
          assets: `Supported assets: ${supportedAssets.join(", ")}`
        });
      }

      // Validate country code if provided
      if (req.country_code && !/^[A-Z]{2}$/.test(req.country_code)) {
        throw new ValidationError("Invalid country code", {
          country_code: "Must be a valid 2-letter ISO country code"
        });
      }
      ...
      if (!rate) {
        throw new ValidationError("Unsupported trading pair", {
          pair: `Trading pair ${pair} is not supported`
        });
      }
