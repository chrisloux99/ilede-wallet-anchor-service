...
/**
 * SEP-24: Interactive withdrawal endpoint.
 * Creates a withdrawal transaction and returns an interactive URL.
 */
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO transactions (stellar_account_id, type, asset_code, amount, status, memo)
       VALUES ($1, 'withdraw', $2, $3, 'pending_user_transfer_start', $4)
       RETURNING id`,
    ...
    return NextResponse.json({
      ...
      url: `${baseUrl}/withdraw-interactive?id=${txId}&account=${encodeURIComponent(account)}`,
      interactive: true,
    });
  } catch (error) {
